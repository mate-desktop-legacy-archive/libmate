...
MATE_CLASS_BOILERPLATE(MateProgram, mate_program, GObject, G_TYPE_OBJECT)

static void global_program_unref(void)
{
    if (global_program)
    {
...
static void mate_program_set_property(GObject* object, guint param_id, const GValue* value, GParamSpec* pspec)
{
	MateProgram* program;
	...
	switch (param_id)
	{
		...
		case PROP_MATE_PATH:
			...
			if (g_value_get_string (value))
				program->_priv->mate_path = g_strsplit(g_value_get_string(value), G_SEARCHPATH_SEPARATOR_S, -1);
			break;
...
static void mate_program_get_property (GObject* object, guint param_id, GValue* value, GParamSpec* pspec)
{
	MateProgram* program;
...
static void add_to_module_list(GPtrArray* module_list, const gchar *module_name)
{
	char** modnames;
	int i, j;
...
static int find_module_in_array(const MateModuleInfo* ptr, MateModuleInfo** array)
{
    int i;
...
/* recursive */
static void mate_program_module_addtolist(MateModuleInfo** new_list, int* times_visited, int* num_items_used, int new_item_idx)
{
	...
	if (times_visited[new_item_idx] > 0)
	{
		 ...
		 */
		g_error("Module '%s' version '%s' has a requirements loop.", new_item->name, new_item->version);
	}
	...
	if (new_item->requirements)
	{
		for (i = 0; new_item->requirements[i].required_version; i++)
		{
			...
			n = find_module_in_array (new_item->requirements[i].module_info, (MateModuleInfo**) program_modules->pdata);
			mate_program_module_addtolist(new_list, times_visited, num_items_used, n);
		}
	}
...
static void mate_program_module_list_order(void)
{
	int i;
	...
	{
		mate_program_module_addtolist(new_list, times_visited, &num_items_used, i);
	}

	/* Now stick the new, ordered list in place */
	memcpy(program_modules->pdata, new_list, program_modules->len * sizeof(gpointer));
}

static void mate_program_class_init(MateProgramClass* klass)
{
	...
	g_object_class_install_property(object_class, PROP_POPT_TABLE, g_param_spec_pointer(MATE_PARAM_POPT_TABLE, _("Popt Table"), _("The table of options for popt"), (G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY)));

	g_object_class_install_property(object_class, PROP_POPT_FLAGS, g_param_spec_int(MATE_PARAM_POPT_FLAGS, _("Popt Flags"), _("The flags to use for popt"), G_MININT, G_MAXINT, 0, (G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY)));

	g_object_class_install_property(object_class, PROP_POPT_CONTEXT, g_param_spec_pointer(MATE_PARAM_POPT_CONTEXT, _("Popt Context"), _("The popt context pointer that MateProgram is using"), (G_PARAM_READABLE)));

	g_object_class_install_property(object_class, PROP_GOPTION_CONTEXT, g_param_spec_pointer(MATE_PARAM_GOPTION_CONTEXT, _("GOption Context"), _("The goption context pointer that MateProgram is using"), (G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY)));

	g_object_class_install_property(object_class, PROP_HUMAN_READABLE_NAME, g_param_spec_string(MATE_PARAM_HUMAN_READABLE_NAME, _("Human readable name"), _("Human readable name of this application"), NULL, (G_PARAM_READABLE | G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY)));

	g_object_class_install_property(object_class, PROP_MATE_PATH, g_param_spec_string(MATE_PARAM_MATE_PATH, _("MATE path"), _("Path in which to look for installed files"), g_getenv ("MATE2_PATH"), (G_PARAM_READABLE | G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY)));

	g_object_class_install_property(object_class, PROP_APP_ID, g_param_spec_string(MATE_PARAM_APP_ID, _("App ID"), _("ID string to use for this application"), NULL, G_PARAM_READABLE));

	g_object_class_install_property(object_class, PROP_APP_VERSION, g_param_spec_string(MATE_PARAM_APP_VERSION, _("App version"), _("Version of this application"), NULL, G_PARAM_READABLE));

	g_object_class_install_property(object_class, PROP_MATE_PREFIX, g_param_spec_string(MATE_PARAM_MATE_PREFIX, _("MATE Prefix"), _("Prefix where MATE was installed"), LIBMATE_PREFIX, (G_PARAM_READABLE | G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY)));

	g_object_class_install_property(object_class, PROP_MATE_LIBDIR, g_param_spec_string(MATE_PARAM_MATE_LIBDIR, _("MATE Libdir"), _("Library prefix where MATE was installed"), LIBMATE_LIBDIR, (G_PARAM_READABLE | G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY)));

	g_object_class_install_property(object_class, PROP_MATE_DATADIR, g_param_spec_string(MATE_PARAM_MATE_DATADIR, _("MATE Datadir"), _("Data prefix where MATE was installed"), LIBMATE_DATADIR, (G_PARAM_READABLE | G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY)));

	g_object_class_install_property(object_class, PROP_MATE_SYSCONFDIR, g_param_spec_string(MATE_PARAM_MATE_SYSCONFDIR, _("MATE Sysconfdir"), _("Configuration prefix where MATE was installed"), LIBMATE_SYSCONFDIR, (G_PARAM_READABLE | G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY)));

	g_object_class_install_property(object_class, PROP_APP_PREFIX, g_param_spec_string(MATE_PARAM_APP_PREFIX, _("MATE App Prefix"), _("Prefix where this application was installed"), NULL, (G_PARAM_READABLE | G_PARAM_WRITABLE)));

	g_object_class_install_property(object_class, PROP_APP_LIBDIR, g_param_spec_string(MATE_PARAM_APP_LIBDIR, _("MATE App Libdir"), _("Library prefix where this application was installed"), NULL, (G_PARAM_READABLE | G_PARAM_WRITABLE)));

	g_object_class_install_property(object_class, PROP_APP_DATADIR, g_param_spec_string(MATE_PARAM_APP_DATADIR, _("MATE App Datadir"), _("Data prefix where this application was installed"), NULL, (G_PARAM_READABLE | G_PARAM_WRITABLE)));

	g_object_class_install_property(object_class, PROP_APP_SYSCONFDIR, g_param_spec_string(MATE_PARAM_APP_SYSCONFDIR, _("MATE App Sysconfdir"), _("Configuration prefix where this application was installed"), NULL, (G_PARAM_READABLE | G_PARAM_WRITABLE)));

	g_object_class_install_property(object_class, PROP_CREATE_DIRECTORIES, g_param_spec_boolean(MATE_PARAM_CREATE_DIRECTORIES, _("Create Directories"), _("Create standard MATE directories on startup"), TRUE, (G_PARAM_READABLE | G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY)));

	g_object_class_install_property(object_class, PROP_ENABLE_SOUND, g_param_spec_boolean(MATE_PARAM_ENABLE_SOUND, _("Enable Sound"), _("Enable sound on startup"), TRUE, (G_PARAM_READABLE | G_PARAM_WRITABLE)));

	g_object_class_install_property(object_class, PROP_ESPEAKER, g_param_spec_string(MATE_PARAM_ESPEAKER, _("Espeaker"), _("How to connect to esd"), NULL, (G_PARAM_READABLE | G_PARAM_WRITABLE)));

	g_type_class_add_private(klass, sizeof(MateProgramPrivate));
}

static void mate_program_instance_init(MateProgram* program)
{
    guint i;
...
static gpointer mate_module_info_copy(gpointer boxed)
{
    return g_memdup(boxed, sizeof(MateModuleInfo));
}

static void mate_module_info_free(gpointer boxed)
{
    g_free(boxed);
}
...
GType mate_module_info_get_type(void)
{
	...
	if (!module_info_type)
		module_info_type = g_boxed_type_register_static("MateModuleInfo", mate_module_info_copy, mate_module_info_free);
...
guint mate_program_install_property(MateProgramClass* pclass, GObjectGetPropertyFunc get_fn, GObjectSetPropertyFunc set_fn, GParamSpec* pspec)
{
	...
	g_object_class_install_property(G_OBJECT_CLASS(pclass), last_property_id, pspec);
...
gchar* mate_program_locate_file(MateProgram* program, MateFileDomain domain, const gchar* file_name, gboolean only_if_exists, GSList** ret_locations)
{
    ...
    if (attr_name != NULL)
    {
		...
		if (dir == NULL)
		{
			g_warning(G_STRLOC ": Directory properties not set correctly.  Cannot locate application specific files.");
			return NULL;
		}

		if (dir != NULL)
		{
			g_snprintf(fnbuf, sizeof(fnbuf), "%s%s/%s", dir, attr_rel, file_name);
    ...
    {
		g_snprintf(fnbuf, sizeof(fnbuf), "%s%s/%s", *ptr, prefix_rel, file_name);
...
static int rpmvercmp(const char* a, const char* b)
{
	...
	while (*one && *two)
	{
		while (*one && !g_ascii_isalnum(*one))
			one++;
		while (*two && !g_ascii_isalnum(*two))
			two++;
		...
		{
			while (*str1 && g_ascii_isdigit(*str1))
				str1++;
			while (*str2 && g_ascii_isdigit(*str2))
				str2++;
		...
		{
			while (*str1 && g_ascii_isalpha(*str1))
				str1++;
			while (*str2 && g_ascii_isalpha(*str2))
				str2++;
...
static gboolean mate_program_version_check(const char* required_version, const char* provided_version)
{
...
void mate_program_module_register(const MateModuleInfo* module_info)
{
    ...
    if (program_modules->len > 0 && g_ptr_array_index(program_modules, program_modules->len - 1) == NULL)
    {
	    g_ptr_array_index(program_modules, program_modules->len - 1) = (MateModuleInfo*) module_info;
    }
    else
    {
    ...
    if (module_info->requirements)
    {
		for (i = 0; module_info->requirements[i].required_version; i++)
		{
			...
			if (mate_program_version_check (module_info->requirements[i].required_version, dep_mod->version))
			{
			...
			/* I18N needed */
				g_error("Module '%s' requires version '%s' of module '%s' to be installed, and you only have version '%s' of '%s'. Aborting application.", module_info->name, module_info->requirements[i].required_version, dep_mod->name, dep_mod->version, dep_mod->name);
			}
		}
    }
}

static void set_context_data(poptContext con, enum poptCallbackReason reason, const struct poptOption* opt, const char* arg, void* data)
{
...
poptContext mate_program_preinit (MateProgram* program, const char* app_id, const char* app_version, int argc, char** argv)
{
	...
	*/
	...
	{
		/* 5b. Create a top-level 'struct poptOption for use in arg-parsing. */
		struct poptOption includer = {NULL, '\0', POPT_ARG_INCLUDE_TABLE, NULL, 0, NULL, NULL};
		 ...
		 * to pass data.  Fun, eh?
		 */
		struct poptOption callback = {NULL, '\0', POPT_ARG_CALLBACK | POPT_CBFLAG_PRE, &set_context_data, 0, NULL, NULL};

		callback.descrip = (const char*) program;

		program->_priv->top_options_table = g_array_new(TRUE, TRUE, sizeof(struct poptOption));
		...
		argctx = program->_priv->arg_context = poptGetContext(program->_priv->app_id, argc, (const char**) argv, (struct poptOption*) program->_priv->top_options_table->data, program->_priv->prop_popt_flags);
	}
	/* 7. Cleanup/return */
 ...
 **/
static char** split_file_list(const char* str)
{
	int i = 0;
	int j;
	char** files;
...
static gchar** get_module_path(void)
{
	...
	if (module_path_env)
		module_path = g_build_path(G_SEARCHPATH_SEPARATOR_S, module_path_env, LIBMATE_LIBDIR, NULL);
	else
		module_path = g_build_path(G_SEARCHPATH_SEPARATOR_S, LIBMATE_LIBDIR, NULL);
...
static gchar* find_module(const gchar* name)
{
	gchar** paths;
	gchar** path;
	gchar* module_name;
...
void mate_program_parse_args(MateProgram* program)
{
	...
	g_return_if_fail((priv->arg_context != NULL && priv->goption_context == NULL) || (priv->arg_context == NULL && priv->goption_context != NULL));
	...
	if (priv->goption_context)
	{
		...
		if (!g_option_context_parse(priv->goption_context, &n_arguments, &arguments, &error))
		{
...
static char* find_accessibility_module (MateProgram* program, const char* libname)
{
	char *sub;
	char *path;
	char *fname;
...
static gboolean accessibility_invoke_module (MateProgram* program, const char* libname, gboolean init)
{
	GModule    *handle;
	void      (*invoke_fn) (void);
	const char *method;
...
static void accessibility_init(MateProgram* program)
{
#ifndef G_OS_WIN32
	int i;
	gboolean do_init;
